...
const updateBook = asyncHandler(async (req, res) => {
    ...
    const newImageLocalPath = req.files?.path;
    console.log(newImageLocalPath);

    if (!newImageLocalPath) {
        throw new ApiError(400, "New book image is required");
    }

    const bookImage = await uploadOnCloudinary(newImageLocalPath);
    if (!bookImage) {
    ...
    const deleteOldImages = book.bookImage.public_id;  
    if (deleteOldImages) {
        await deleteOnCloudinary(deleteOldImages);
    }
